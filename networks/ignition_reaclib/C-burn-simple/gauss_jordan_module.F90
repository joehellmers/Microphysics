...
contains
  ...
  subroutine gauss_jordan_solve(A, x, b)
    ...
    scratch_3 = A(1,4)*scratch_0
    scratch_4 = 1.0/A(3,3)
    scratch_5 = A(3,4)*scratch_4
    scratch_6 = 1.0/(-A(4,3)*scratch_5 + A(4,4))
    scratch_7 = A(3,9)*scratch_4
    scratch_8 = scratch_6*(-A(4,3)*scratch_7 + A(4,9))
    scratch_9 = 1.0/A(2,2)
    scratch_10 = scratch_9*(-A(2,1)*scratch_2 + A(2,9))
    scratch_11 = 1.0/A(5,5)
    scratch_12 = -A(5,3)*scratch_5 + A(5,4)
    scratch_13 = scratch_11*(-A(5,3)*scratch_7 + A(5,9) - scratch_12*scratch_8)
    scratch_14 = 1.0/A(6,6)
    scratch_15 = scratch_14*(-A(6,4)*scratch_8 + A(6,9))
    scratch_16 = 1.0/A(7,7)
    scratch_17 = scratch_16*(-A(7,4)*scratch_8 + A(7,9))
    scratch_18 = 1.0/A(8,8)
    scratch_19 = scratch_18*(-A(8,4)*scratch_8 + A(8,9))
    scratch_20 = scratch_9*(-A(2,1)*scratch_3 + A(2,4))
    scratch_21 = -A(9,1)*scratch_3 - A(9,2)*scratch_20 - A(9,3)*scratch_5 + A(9,4)
    scratch_22 = scratch_9*(-A(2,1)*scratch_1 + b(2))
    scratch_23 = b(3)*scratch_4
    scratch_24 = scratch_6*(-A(4,3)*scratch_23 + b(4))
    ...
    scratch_29 = (-A(9,1)*scratch_1 - A(9,2)*scratch_22 - A(9,3)*scratch_23 - A(9,5)* &
      ...
      A(9,2)*scratch_10 - A(9,3)*scratch_7 - A(9,5)*scratch_13 - A(9,6) &
      *scratch_15 - A(9,7)*scratch_17 - A(9,8)*scratch_19 + A(9,9) - &
      scratch_21*scratch_8)
    scratch_30 = -A(10,1)*scratch_3 - A(10,2)*scratch_20 - A(10,3)*scratch_5 + A(10,4)

    x(1) = scratch_1 - scratch_24*scratch_3 - scratch_29*(scratch_2 - scratch_3* &
      scratch_8)
    x(2) = -scratch_20*scratch_24 + scratch_22 - scratch_29*(scratch_10 - &
      scratch_20*scratch_8)
    x(3) = scratch_23 - scratch_24*scratch_5 - scratch_29*(-scratch_5*scratch_8 + &
      scratch_7)
    x(4) = scratch_24 - scratch_29*scratch_8
    ...
    x(10) = (-A(10,1)*scratch_1 - A(10,2)*scratch_22 - A(10,3)*scratch_23 - A(10,5)* &
      ...
      *scratch_2 - A(10,2)*scratch_10 - A(10,3)*scratch_7 - A(10,5)* &
      scratch_13 - A(10,6)*scratch_15 - A(10,7)*scratch_17 - A(10,8)* &
      scratch_19 + A(10,9) - scratch_30*scratch_8))/A(10,10)
