...
program test_react
  ...
  do i = 1, nfabs(s(n))
     ...
     do nn = 1, cuNumStreams
        ...
        !blockShmemBytes = 8*cuThreadBlock*(nspec+2)**2
        blockShmemBytes = 0
        call react_zones<<<cuGrid, cuThreadBlock, blockShmemBytes, cuStreams(nn)>>>(state_flat_dev, &
             chunkOffset, chunkSize, statePitch, stateLength)        
     end do
